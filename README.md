...
게시글 작성자가 AI 답변을 요청하면 서버는 즉시 taskId를 반환하고, LLM 호출·댓글 저장은 별도 비동기 스레드풀(llmExecutor)에서 처리합니다. 프론트엔드는 1초 간격 폴링으로 생성 상태(PENDING → COMPLETED / FAILED)를 확인하여 결과를 표시합니다. DB 원자적 UPDATE 기반 락으로 동시 요청을 제어하고 LLM 호출 성공 후에만 횟수를 차감합니다. 유저당 하루 3회 사용 제한.
